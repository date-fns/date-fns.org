...
export default class Perf extends React.Component {
    ...
    return <div className='perf'>
      <div>
        {perfStats.map((build, index) => (
          <button type='button' onClick={this._switchTo.bind(this, index)} key={index}>
            {build.buildNumber}
          </button>
        ))}
      </div>
  ...
  _renderChart () {
    ...
    return fns.map(fn => {
      ...
      return <div className='perf-row' key={fn}>
        <div className='perf-fn'>
          {fn}
        </div>
        <div className='perf-bars'>
          <div className='perf-bar is-date_fns' style={{width: `${dateFns / biggestNumber * 100}%`}}>
            {dateFns} ops/sec
          </div>
          {typeof moment === 'number'
            ? <div className='perf-bar is-moment' style={{width: `${moment / biggestNumber * 100}%`}} />
            : null}
        </div>
      </div>
    })
